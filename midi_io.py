...
_SETTINGS_PATH = None  # Will be set by platform module
...
APP_FULL_NAME = "Midi-Studio — a Synthesizer in the Spirit of MidiSoft Studio4 (Work-Alike: No code from original)"
APP_TITLE     = f"{APP_FULL_NAME}  —  v{APP_VERSION}  {APP_TIMESTAMP}"

# ─────────────────────────────────────────────────────────────────────────────
# Sterling Lions Club links  (update URLs as needed)
# ─────────────────────────────────────────────────────────────────────────────
...
if not MIDI_OUT_OK and not _fs_active and _unverified_out_port_name:
    try:
        ...
        print(f"[MIDI OUT] Last-resort fallback also failed: {_fallback_exc}",
              file=sys.stderr)
# ─────────────────────────────────────────────────────────────────────────────
# Single-thread MIDI input dispatcher
# ─────────────────────────────────────────────────────────────────────────────
# All MIDI input is funnelled through ONE thread that owns _midi_in exclusively.
# Listeners register a callback; the dispatcher fans out each message instantly.
# This eliminates the race between the monitor thread and the record thread,
# and keeps key→audio latency as low as possible (~1 ms poll).
#
# Usage:
#   token = midi_input_subscribe(my_callback)   # callback(msg) called in dispatcher thread
#   midi_input_unsubscribe(token)
